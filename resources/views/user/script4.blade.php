<script>
    ...
    function hapusCoverCard2(cardId, event) {
        ...
        $.ajax({
            ...
            success: function(response) {
                toastr.success('Anda berhasil menghapus cover!');
                updateCoverCard('NULL', cardId);
                updateTextCover(cardId, 'NULL');
            },
            error: function(error) {
                toastr.error('Anda gagal menghapus cover!');
            }
        });
    }
    ...
    function updateTextCover(cardId, cover) {
        ...
        if (linkElement) {
            if (cover === 'NULL' || cover === '') {
                linkElement.innerHTML = '<i class="fa-solid fa-clapperboard m-r-5"></i> Tambah Cover';
            } else {
                linkElement.innerHTML = '<i class="fa-solid fa-clapperboard m-r-5"></i> Perbaharui Cover';
            }
        }
    }
    ...
    $(document).ready(function() {
        ...
        if ('{{ $isianKartu->pattern }}') {
            coverTextElement.textContent = 'Perbaharui Cover';
        }
    });
</script>