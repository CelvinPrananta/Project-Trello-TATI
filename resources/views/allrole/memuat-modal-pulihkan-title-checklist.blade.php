                            ...
                            '<li class="pulihkan-title" id="pulihkan-title-' + titleChecklist.id + '">' +
                                '<div class="isian-pulihkan-title">' + titleChecklist.name +
                                    ...
                                    '<div class="info-status14">' +
                                        ...
                                        '</button>' +
                                    '</div>' +
                                    '<div class="waktu-pulihkan-title">Dihapus ' + waktuPulihkanKartu + '</div>' +
                                '</div>' +
                            '</li>';
                    });
                            ...
                            '<li class="pulihkan-checklist" id="pulihkan-checklist-' + checklist.id + '">' +
                                '<div class="isian-pulihkan-checklist">' + '<p class="text-from-judul">' + 'from ' + checklist.title_checklist_name + '</p>' + checklist.name +
                                    ...
                                    '<div class="info-status16">' +
                                        ...
                                        '</button>' +
                                    '</div>' +
                                    '<div class="waktu-pulihkan-checklist">Dihapus ' + waktuPulihkanKartu + '</div>' +
                                '</div>' +
                            '</li>';
                    });
...
// ---------------------------------------------------------- Pemulihan Data Judul Checklist ---------------------------------------------------------- //
    ...
    function pulihkanTitle(cardId) {
        $.post('{{ route("pulihkanJudulChecklist") }}', { id: cardId, _token: '{{ csrf_token() }}' }, function(response) {
            if (response.message === 'Anda berhasil memulihkan judul checklist!') {
                ...
                toastr.error('Anda gagal memulihkan judul checklist!');
            }
        }).fail(function() {
            toastr.error('Anda gagal memulihkan judul checklist!');
        });
            ...
            <div class="menu-checklist border border-1 border-darkss p-2 rounded-xl" data-id="${response.titlechecklist.id}">
                ...
                <div class="header-checklist flex justify-content">
                    ...
                    <form id="myFormTitleUpdate${response.titlechecklist.id}" method="POST" class="update-title">
                        ...
                        <input type="text" class="isian-title border border-1 border-darks w-402 p-2 rounded-xl" style="font-size: 17px" id="titleChecklistUpdate${response.titlechecklist.id}" name="titleChecklistUpdate" placeholder="Masukkan Judul" value="${response.titlechecklist.name}">
                        <div class="aksi-update-title gap-2">
                            <button type="submit" class="btn btn-outline-info icon-keterangan hidden" id="saveButtonTitleUpdate${response.titlechecklist.id}">Simpan</button>
                            <button type="button" class="btn btn-outline-danger icon-keterangan hidden" id="cancelButtonTitleUpdate${response.titlechecklist.id}">Batal</button>
                        </div>
                    </form>
                ...
                html += `
                    <div id="section-checklist-${checklist.id}" class="input-checklist" data-id="${checklist.id}">
                        <form id="myFormChecklistUpdate${checklist.id}" method="POST" class="form-checklist">
                            ...
                            <input onclick="mentionTags4('checkbox-${checklist.id}')" type="text" class="dynamicCheckboxValue border border-1 border-darks w-402 p-2 rounded-xl hidden" id="checkbox-${checklist.id}" name="checkbox-${checklist.id}" value="${checklist.name}" placeholder="Masukkan Checklist">
                            ...
                            <div onclick="checklistUpdate(${checklist.id})" class="aksi-update-checklist2 gap-2 margin-bottom-0" id="checklist-${checklist.id}">
                                <button type="button" class="saves btn btn-outline-info hidden" id="saveButtonChecklistUpdate-${checklist.id}">Simpan</button>
                                <button type="button" class="cancels btn btn-outline-danger hidden" id="cancelButtonChecklistUpdate-${checklist.id}">Batal</button>
                            </div>
                        </form>
        ...
        html += `
                ...
                <form id="myFormChecklist${response.titlechecklist.id}" method="POST">
                    ...
                    <div class="header-tambah-checklist flex gap-4">
                        <i class="fa-xl"></i>
                        <input onclick="mentionTags('checklist${response.titlechecklist.id}')" type="text" class="tambah-baru-checklist border border-1 border-dark w-407s p-2 rounded-xl hidden" id="checklist${response.titlechecklist.id}" name="checklist" placeholder="Masukkan nama checklist" required>
                        <div class="mention-tag" id="mention-tag-checklist${response.titlechecklist.id}"></div>
                    </div>
                    <div class="aksi-update-checklist gap-2">
                        <button type="submit" class="btn btn-outline-info icon-keterangan hidden" id="saveButtonChecklist${response.titlechecklist.id}">Simpan</button>
                        <button type="button" class="btn btn-outline-danger icon-keterangan hidden" id="cancelButtonChecklist${response.titlechecklist.id}">Batal</button>
                    </div>
                </form>

                <button type="button" class="btn btn-outline-info" id="AddChecklist${response.titlechecklist.id}"><i class="fa-solid fa-plus" aria-hidden="true"></i> Tambah Item...</button>
                <!-- Tambah baru checklist -->
        ...
        $(document).ready(function() {
            ...
            $(`#myFormTitleUpdate${titleId}`).on('submit', function(event) {
                ...
                $.ajax({
                    ...
                    error: function(error) {
                        toastr.error('Anda gagal memperbaharui judul!');
            ...
            $(document).on('submit', '[id^="myFormTitleDelete"]', function(event) {
                ...
                $.ajax({
                    ...
                    error: function(error) {
                        toastr.error('Anda gagal menghapus judul!');
            ...
            $(`#myFormChecklist${titleId}`).on('submit', function(event) {
                ...
                $.ajax({
                        ...
                        var newForm = `
                            <div id="section-checklist-${response.checklist.id}" class="input-checklist" data-id="${response.checklist.id}">
                                <form id="myFormChecklistUpdate${response.checklist.id}" method="POST" class="form-checklist">
                                    ...
                                    <input type="hidden" id="card_id" name="card_id" value="${response.titlechecklist.cards_id}">
                                    <input onclick="mentionTags4('checkbox-${response.checklist.id}')" type="text" class="dynamicCheckboxValue border border-1 border-darks w-402 p-2 rounded-xl hidden" id="checkbox-${response.checklist.id}" name="checkbox-${response.checklist.id}" value="${response.checklist.name}" placeholder="Masukkan Checklist">
                    ...
                    error: function(error) {
                        toastr.error('Anda gagal membuat checklist!');
            ...
            $(document).on('click', '.saves', function(event) {
                ...
                $.ajax({
                    ...
                    error: function(error) {
                        toastr.error('Anda gagal memperbaharui checklist!');
            ...
            $(`#checklistform-all-${titleId}`).on('change', function() {
                ...
                var toastBerhasil = isChecked ? 'Anda berhasil centang semua checklist!' : 'Anda berhasil tidak centang semua checklist!';
                ...
                $.ajax({
                    ...
                    error: function(xhr) {
                        toastr.error('Anda gagal memperbarui checklist!');
    ...
    function deleteTitle(cardId) {
        $.ajax({
            ...
            success: function(response) {
                if (response.message === 'Anda berhasil menghapus judul checklist permanen!') {
                    toastr.success(response.message);
                    ...
                    toastr.error('Anda gagal menghapus judul checklist!');
                }
            }
        }).fail(function() {
            toastr.error('Anda gagal menghapus judul checklist!');
...
// ---------------------------------------------------------- Pemulihan Data Checklist ---------------------------------------------------------- //
    ...
    function pulihkanChecklist(cardId) {
        $.post('{{ route("pulihkanChecklist") }}', { id: cardId, _token: '{{ csrf_token() }}' }, function(response) {
            if (response.message === 'Anda berhasil memulihkan checklist!') {
                toastr.success(response.message);
                ...
                toastr.error('Anda gagal memulihkan checklist!');
            }
        }).fail(function() {
            toastr.error('Anda gagal memulihkan checklist!');
        });
    ...
    function deleteChecklist(cardId) {
        $.ajax({
            ...
            data: { id: cardId, _token: '{{ csrf_token() }}' },
            success: function(response) {
                if (response.message === 'Anda berhasil menghapus checklist permanen!') {
                    toastr.success(response.message);
    ...
    function pindahTitleDanChecklist() {
        $(document).ready(function(){
            ...
            function updateTitlePositions() {
                ...
                .then(data => {
                    if (data.success) {
                        toastr.success('Anda berhasil perbaharui posisi judul!');
                    } else {
                        toastr.error('Anda gagal perbaharui posisi judul!');
                    }
                })
            ...
            function updateChecklistPositions() {
                ...
                .then(data => {
                    if (data.success) {
                        toastr.success('Anda berhasil perbaharui posisi checklist!');
                    } else {
                        toastr.error('Anda gagal perbaharui posisi checklist!');
                    }
