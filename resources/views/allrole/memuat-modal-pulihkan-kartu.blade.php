<script>
                        ...
                        list += '<li class="pulihkan-kartu" id="pulihkan-kartu-' + card.id + '">' +
                                    '<div class="isian-pulihkan-kartu">' + card.name + 
                                        ...
                                        '<div class="info-status18">' +
                                            ...
                                            '</button>' +
                                        '</div>' +
                                    '<div class="waktu-pulihkan-kartu">Dihapus ' + waktuPulihkanKartu + '</div>' + '</div>' +
                                '</li>';
                    });
    ...
    function pulihkanKartu(cardId) {
        $.post('{{ route("pulihkanKartu") }}', { id: cardId, _token: '{{ csrf_token() }}' }, function(response) {
            if (response.message === 'Anda berhasil memulihkan kartu!') {
                ...
                toastr.error('Anda gagal memulihkan kartu!');
            }
        }).fail(function() {
            toastr.error('Anda gagal memulihkan kartu!');
        });
    ...
    function pulihkanKartuContainer(response) {
        ...
        new_card.classList.add('kartu-trello');
                    ...
                    <div class="tampilan-info gap-2">
                        ...
                        <div id="iconChecklist-${card.id}" class="progress-checklist-light hidden @foreach($result_tema as $mode_tema) @if ($mode_tema->tema_aplikasi == 'Gelap') progress-checklist-dark hidden @endif @endforeach">
                            <div class="info-status9">
                                ...
                                ${card.description ? `<span class="text-status9"><b>Item Checklist</b></span>` : `<span class="text-status9a"><b>Item Checklist</b></span>`}
    ...
    function deleteKartu(cardId) {
        $.ajax({
            ...
            success: function(response) {
                if (response.message === 'Anda berhasil menghapus kartu permanen!') {
                    toastr.success(response.message);
