        ...
        $('#makeTemplateForm' + id).on('submit', function(event){
            ...
            $.ajax({
                ...
                success: function(response){
                    ...
                    $('#iconCheck-' + id).addClass('hidden');
                    $('#addTitle-' + id).removeClass('hidden');
                    toastr.success('Anda berhasil membuat template judul!');
                                ...
                                <div class="header-checklist flex justify-content">
                                    ...
                                    <form id="myFormTitleUpdate${titlechecklist.id}" method="POST" class="update-title">
                                        @csrf
                                            ...
                                            <input type="text" class="isian-title border border-1 border-darks w-402 p-2 rounded-xl" style="font-size: 17px" id="titleChecklistUpdate${titlechecklist.id}" name="titleChecklistUpdate" placeholder="Masukkan Judul" value="${titlechecklist.name}">
                                            <div class="aksi-update-title gap-2">
                                                <button type="submit" class="btn btn-outline-info icon-keterangan hidden" id="saveButtonTitleUpdate${titlechecklist.id}">Simpan</button>
                                                <button type="button" class="btn btn-outline-danger icon-keterangan hidden" id="cancelButtonTitleUpdate${titlechecklist.id}">Batal</button>
                                            </div>
                                    </form>
                                ...
                                <form id="myFormChecklist${titlechecklist.id}" method="POST">
                                    @csrf
                                        ...
                                        <div class="header-tambah-checklist flex gap-4">
                                            <i class="fa-xl"></i>
                                            <input onclick="mentionTags('checklist${titlechecklist.id}')" type="text" class="tambah-baru-checklist border border-1 border-dark w-407s p-2 rounded-xl hidden" id="checklist${titlechecklist.id}" name="checklist" placeholder="Masukkan nama checklist" required>
                                            <div class="mention-tag" id="mention-tag-checklist${titlechecklist.id}"></div>
                                        </div>
                                        <div class="aksi-update-checklist gap-2">
                                            <button type="submit" class="btn btn-outline-info icon-keterangan hidden" id="saveButtonChecklist${titlechecklist.id}">Simpan</button>
                                            <button type="button" class="btn btn-outline-danger icon-keterangan hidden" id="cancelButtonChecklist${titlechecklist.id}">Batal</button>
                                        </div>
                                </form>

                                <button type="button" class="btn btn-outline-info" id="AddChecklist${titlechecklist.id}"><i class="fa-solid fa-plus" aria-hidden="true"></i> Tambah Item...</button>
                                <!-- Tambah baru checklist -->
                ...
                error: function (xhr, status, error) {
                    toastr.error('Anda gagal membuat template judul!');
                }
            });
        });
        // End Section Title
