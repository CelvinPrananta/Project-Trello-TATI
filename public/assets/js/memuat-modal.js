...
function copyCardModal(cardId, cardName, formAction) {
    ...
    $.ajax({
        ...
        error: function(error) {
            toastr.error('Anda gagal mengambil total checklist!');
        }
    });
}
    ...
    $('#updateCardForm').on('submit', function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                toastr.success('Anda berhasil memperbaharui kartu!');
            },
            error: function(response) {
                if (response.responseJSON && response.responseJSON.errors) {
                    var errors = response.responseJSON.errors;
                    ...
                    toastr.error('Anda gagal memperbaharui kartu!');
                }
            }
        });
    });
    ...
    $('#copyCardForm').on('submit', function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                            ...
                            <div class="tampilan-info gap-2">
                                ...
                                <div id="iconChecklist-${response.card.id}" class="progress-checklist-light hidden @foreach($result_tema as $mode_tema) @if ($mode_tema->tema_aplikasi == 'Gelap') progress-checklist-dark hidden @endif @endforeach">
                                    <div class="info-status9">
                                        ...
                                        ${response.card.description ? `<span class="text-status9"><b>Item Checklist</b></span>` : `<span class="text-status9a"><b>Item Checklist</b></span>`}
                ...
                toastr.success('Anda berhasil menyalin kartu!');
                $('#copyCard').modal('hide');
            ...
            error: function(xhr, status, error) {
                toastr.error('Anda gagal menyalin kartu!');
    ...
    function sendMentions(users, checklistName) {
        ...
        Promise.all(promises)
            .then(responses => {
                ...
                if (allSuccessful) {
                    console.log('Anda berhasil mengirimkan mention tag!');
                } else {
